...
<template>
    <ul>
        <li v-for="attribute in type.available_attributes.filter(attr => (attr.display === true && display === true) || (attr.show_off === true && show_off === true))"
            :key="attribute.code">
            <b>{{ attribute.name }}:</b> <span v-if="Array.isArray(object.attributes[attribute.code])">
