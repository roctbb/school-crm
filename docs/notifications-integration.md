...
Все пользователи с ролью `admin` автоматически получают уведомления о днях рождения
подтверждённых, неудалённых учеников. Для каждого именинника приходит отдельное
сообщение с именем и ссылкой на карточку ученика — по email и в подключённый Telegram.
Включать публичный экран дней рождения для этого не требуется.

Рассылка начинается в 09:00 по `SCHOOL_TIMEZONE` (по умолчанию `Europe/Moscow`).
Сервис `scheduler` проверяет даты каждые пять минут: после простоя или добавления
ученика уведомление будет отправлено при ближайшей проверке в тот же день.
Повторные проверки не создают новые уведомления и не повторяют успешно завершённую
доставку; незавершённые каналы ставятся в очередь снова. За прошлые дни рассылки нет.
День рождения 29 февраля учитывается только 29 февраля в високосные годы.

При обновлении примените миграцию `ae91b38d720f` (`flask db upgrade`; штатный запуск
backend в Docker выполняет её автоматически) и запустите планировщик вместе с worker:

```bash
docker compose up -d --build backend worker scheduler
```

Должен работать один экземпляр `scheduler`.
